...
const _transformCirclePieData = (data, indicators) => {
  data.forEach((item) => {
  ...
  // 饼图的标签样式
  const _attachPieLabelStyle = (series, options, dataOrigin) => {
...
class CirclePie extends React.Component {
  static propTypes = {
  ...
  constructor(props) {
    ...
    this.state = {
      msg: '环形饼图',
      data: _transformCirclePieData(data, indicators),
      seriesTmpl: {
        data: [],
        cursor: 'auto',
        selectedMode: 'single',
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(!_.isEqual(this.props.data, nextProps.data)) {
      this.setState({
        data: _transformCirclePieData(nextProps.data.data, nextProps.data.indicators)
      })
    }
  ...
  bindEvents(){
    ...
    if(through) {
      ...
      const graphConnectClickEvent = (params) => {
        ...
        if(this.validConnect) {   // 判断是否联动
          this._handleChange(params)
        }
      }
      if(!this.hasBindClickEvent) {  // 判断是否绑定事件
  ...
  getOptions() {
    ...
    const { data } = this.state;
    const chartData = this._converData();
  ...
  _handleChange(params){
    //判断是否已经是处于选中状态 发起单图联动拼接conditions
    const { chartRelated, currentName } = this.connectStore;
    const { currentRelatedChartId, chartId, events, designTime } = this.props;
    const { data } = this.state;

    const { dimsForRelated } = data;
    const conditions = [];
    // 判断当前筛选的数据集currentRelatedChartId 不为空 或者chartId === currentRelatedChartId 就进行下一步
    if ((!currentRelatedChartId || currentRelatedChartId ===chartId) && dimsForRelated) {
      this.hasBindClickEvent = true; // 是否绑定事件
      this.validConnect = false;    // 是否联动有效
...
export default Connect()(CirclePie)

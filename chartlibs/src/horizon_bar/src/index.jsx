...
class HorizonBar extends React.Component {
  static propTypes = {
  ...
  getOptions(dataOrigin) {
    ...
    //如果不存在目标值则设置最大值
    if (!Array.isArray(indicators.desires) || indicators.desires.length === 0) {
  ...
  _convertData(dataOrigin) {
    ...
    let desireLen = 0
    const maxV = __getNumsMax(Object.assign({}, nums, desire), false)
    // 模拟背景色柱子
  ...
  _applyColorStyle(chartData) {
    ...
    if (colorTheme) {
      ...
      series.forEach((sery, i) => {
        if (i <= lastIndex) {
          _attachColorStyle(sery, colorTheme, i, lastIndex + 1, relatedIndex)
        }
      })
    }
    return chartData
...
export default Connect()(HorizonBar)

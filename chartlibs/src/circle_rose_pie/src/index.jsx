...
const _transformCircleRosePieData = (data, indicators) => {
  data.forEach((item) => {
...
// 标签样式
const _attachCircleRosePieLabelStyle = (series, options, dataOrigin) => {
...
class CircleRosePie extends React.Component {
  static propTypes = {
  ...
  constructor(props) {
    ...
    this.state = {
      data: _transformCircleRosePieData(data, indicators),
      msg: '环形玫瑰图',
      seriesTmpl: {
        data: [],
        cursor: 'auto',
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_.isEqual(this.props.data, nextProps.data)) {
      this.setState ({
        data: _transformCircleRosePieData(nextProps.data.data, nextProps.data.indicators)
      })
    }
  ...
  _converData() {
    ...
    seriesTmpl.data = seriesData.map((item, i) => {
      ...
      _attachCircleRosePieLabelStyle(dataItem, config.global, data);
  ...
  _handleChange(params) {
    ...
    const conditions = [];
     //如果当前数据集筛选中currentRelatedChartId为空 或者chartId等于currentRelatedChartId就进行逻辑, 并且维度存在的情况下
...
export default Connect()(CircleRosePie)

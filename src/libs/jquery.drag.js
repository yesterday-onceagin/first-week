...
(function (factory) {
  ...
  $.fn.arrangeable = function(options) {
    ...
    this.each(function() {
      ...
      if (dragSelector) {
        $this.
          off(dragEvents.START + eventNamespace).
          on(dragEvents.START + eventNamespace, dragSelector, dragStartHandler);
      } else {
        $this.
          off(dragEvents.START + eventNamespace).
          on(dragEvents.START + eventNamespace, dragStartHandler);
      }
    ...
    $(document).
      off(dragEvents.MOVE + eventNamespace).
      off(dragEvents.END + eventNamespace).
      on(dragEvents.MOVE + eventNamespace, dragMoveHandler).
    ...
    function dragMoveHandler(e) {
      ...
      // check for drag threshold (drag has not started yet)
        ...
        // 去除 reactid 属性. 避免react-matation对dom树的监听. 已造成不必要的报错
        $clone.removeAttr('data-reactid')
        $clone.find('i').removeAttr('data-reactid')
        $clone.find('p').removeAttr('data-reactid')
        $clone.find('div').removeAttr('data-reactid')
  ...
  function clone($element) {
    ...
    var paddingLeft = +$element.css('paddingLeft').replace('px','')
    var paddingTop = +$element.css('paddingTop').replace('px','')

    $clone.css({
      position: 'absolute',
      width: $element.width() + paddingLeft * 2 + 2,
      height: $element.height() + paddingTop * 2 + 2,
  ...
  function getHoveredElement($clone, $dragElement, $movableElements) {
    ...
    for (var i = 0; i < $movableElements.length; i++) {
      ...
      var paddingLeft = +$currentElement.css('paddingLeft').replace('px','')
      var paddingTop = +$currentElement.css('paddingTop').replace('px','')

      horizontalMidPosition = offset.left + 0.5 * $currentElement.width() + paddingLeft + 1;
      verticalMidPosition = offset.top + 0.5 * $currentElement.height() + paddingTop + 1;

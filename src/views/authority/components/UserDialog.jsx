...
class UserDialog extends React.Component {
    ...
    if (!_.isEqual(this.props.data, nextProps.data)) {
      this.setState({
        info: {
          ...
          role_ids: nextProps.data.role_ids || []
        }
      })
    }
  }
  ...
  render() {
    ...
    const dialogHeight = passwordOnly ? '214px' : (!data.id ? '584px' : '510px')
    ...
    return (
      <div>
        ...
        <div className="form-group">
          <label className="control-label"><i className="required">&nbsp;</i>选择角色</label>
          <div className="input-wrapper">
            <Select type="multiple" value={info.role_ids} maxHeight={200} width="100%" openSearch hasIcon showMultipleBar={false} onSelected={this.handleChangeRole.bind(this, 'role_ids')}>
              {
                roleList.map((item, key) => <option key={key} value={item.id}>{item.name}</option>)
              }
            </Select>
          </div>
        </div>
  ...
  handleGo = (e) => {
    e.stopPropagation();
    browserHistory.push('/authority/add')
  };

  handleChangeRole = (field, option) => {

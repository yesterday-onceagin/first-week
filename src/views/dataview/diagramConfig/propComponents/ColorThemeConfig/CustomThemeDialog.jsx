...
import '../../components/dataSeries/color-theme-dialog.less'
import '../../components/dataSeries/color-picker.less'

const _rgbaToString = function (rgb) {
  return `rgba(${rgb.r},${rgb.g},${rgb.b},${rgb.a})`
}

const DEFAULT_LEGEND_TYPES = ['bar_like', 'pie_like', 'scatter_like', 'line_like']

const getColNames = function (dataArr = []) {
  return dataArr.map(item => item.col_name)
}

class CustomThemeDialog extends React.Component {
  static defaultProps = {
    gradient: true,
    legends: 'bar_like',
  ...
  static propTypes = {
    gradient: PropTypes.bool,                   // 是否支持渐变色
    dimensionSwitch: PropTypes.bool,            // 是否支持数值和维度切换
    legends: (props, propName) => {
      const v = props[propName]
      if (v && typeof v !== 'function' && DEFAULT_LEGEND_TYPES.indexOf(v) === -1) {
        return new Error(`不支持的legends类型: 只能是${DEFAULT_LEGEND_TYPES.toString()} 或者 一个返回list的函数 或者为空`)
      }
    },    // 'bar_like', 'pie_like', func
  ...
  render() {
    const { chart, gradient } = this.props
                  ...
                  <div>
                    <div ref={(node) => { this.picker_popver_container = node }}>
                      <OverlayTrigger
                        ...
                        onEnter={this.handleColorpickEntered.bind(this)}
                      >
                        <span className="start-color" style={{ background: currentColorAtLegend[0] }} />
                      </OverlayTrigger>
                      <span className="color-result" style={{ background: `linear-gradient(90deg, ${currentColorAtLegend[0]}, ${currentColorAtLegend[1]})` }} />
                      <OverlayTrigger
                        trigger="click"
                        placement="bottom"
                        rootClose
                        ...
                        onEnter={this.handleColorpickEntered.bind(this)}
                      >
                        <span className="end-color" style={{ background: currentColorAtLegend[1] }} />
                      </OverlayTrigger>
                    </div>
                  </div>
  ...
  renderAffectButton() {
    const { /* chart,  */dimensionSwitch } = this.props
    const { colorThemeCopy } = this.state
    const hasAffect = dimensionSwitch
  ...
  _getLegends() {
    const { chart, chartData, legends } = this.props
    const { colorThemeCopy } = this.state
    let legendList = []
    const affectDim = +colorThemeCopy.affect === 1
    if (chart) {
      if (typeof legends === 'function') {
        return legends(chart, chartData, affectDim)
      }
      const index = DEFAULT_LEGEND_TYPES.indexOf(legends)
      console.log(index, '1231231232353646')
      switch (index) {
        case 1: {
          const colName = chart.dims[0] && chart.dims[0].col_name
          // 一个维度, 一个数值
          if (colName) {
            legendList = chartData && Array.isArray(chartData) ? chartData.map(item => item[colName]) : []
          } else {   // 0个维度 多个数值
            legendList = chart.nums.map(item => item.alias || item.text)
          }
          break
        }
        case 2:
          if (affectDim) {
            ...
            chartData.forEach((dataItem) => {
              ...
              legendList.push(_name)
            })
          } else {
            legendList = chartData && chartData.length > 0 ? ['散点颜色'] : []
          }
          break
        case 3:
          legendList = chart.nums.map(item => item.alias || item.text)
            ...
            chartData.forEach((dataItem) => {
              ...
              if (chart.nums.length > 1) {
                chart.nums.forEach((num) => {
                  legendList.push(`${strArr}_${num.alias || num.text}`)
                })
              } else {
                legendList.push(strArr)
              }
            })
          } else {
            const zaxisList = chart.zaxis ? chart.zaxis.map(item => item.alias || item.text) : []
            legendList = chart.nums.map(item => item.alias || item.text)
            legendList = legendList.concat(zaxisList)
          }
          break
      }
    }
    ...
    if (chart) {
      switch (chart.chart_code) {
        ...
        case 'scatter_map':
          legendList = chartData && chartData.length > 0 ? ['散点颜色', '地图颜色', '轮廓颜色'] : []
          break
        default:
          break
      }

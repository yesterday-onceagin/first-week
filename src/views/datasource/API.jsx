...
const API = createReactClass({
  ...
  getInitialState() {
    return {
      info: {
        type: DATA_SOURCE_TYPES.api.type,
        conn_str: {
          // 测试使用的API
  ...
  componentWillMount() {
    ...
    onChangeNavBar([{
      ...
      name: `${params.datasource_id ? '修改' : '添加'}${DATA_SOURCE_TYPES.api.name}数据源`
    }]);
  },
  ...
  // 渲染API参数配置部分
  renderParamOptions() {
    const { apiDatasourceParamsPending, apiDatasourceParams, apiDatasourceSysParams } = this.props
    ...
    return (
      <div className="form-group-set no-border">
        ...
        <div className="form-group-row">
          <KeyValueInput
            style={{ padding: '36px 0 4px 12px' }}
            pending={apiDatasourceParamsPending}
            emptyText="请先完成接口配置"
            data={info.conn_str.params}
  ...
  handleChangeApiOpts(field, e) {
    ...
    const oldHost = _.get(oldInfo, 'conn_str.host')
    const oldAccessSecret = _.get(oldInfo, 'conn_str.access_secret')
    // 持续输入时清除调用
    clearTimeout(paramFetchTimer)
    this.setState(preState => ({
      ...
      const { info } = this.state
      const host = _.get(info, 'conn_str.host')
      const accessSecret = _.get(info, 'conn_str.access_secret')
      // 如果完成了填写 并且有变更 1秒后调用接口
      if (host && accessSecret && (oldHost !== host || oldAccessSecret !== accessSecret)) {
        paramFetchTimer = setTimeout(() => {
          this._fetchApiParams(info)
        }, 1000)
      }
    })
  },
  ...
  handleValidSubmit() {
    ...
    // 参数配置项检查
    let errorMsg = ''
    // 剔除没有选中的参数 并且过滤checked属性
    sendParams.conn_str.params = _.filter(sendParams.conn_str.params, item => item.checked).map((item) => {
      if (item.checked !== undefined) {
        Reflect.deleteProperty(item, 'checked')
      }
      return item
    })
    _.every(sendParams.conn_str.params, (item) => {
      // 如果是固定参数 则检查是否填写了默认值
      if (item.type === 'fixed') {
...
export default connect(stateToProps, dispatchToProps)(API);

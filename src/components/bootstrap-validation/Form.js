...
export default class Form extends InputContainer {
  ...
  _renderChild(child) {
    ...
    if (child.type === ValidatedInput ||
      ...
      const evtName = child.props.validationEvent ?
        child.props.validationEvent : this.props.validationEvent;

      const origCallback = child.props[evtName];
      ...
      const error = this._hasError(name);
  ...
  _validateOne(iptName, context) {
    const input = this._inputs[iptName];
    ...
    const value = context[iptName];
    let isValid = true;
    const validate = input.props.validate;
    let result,
      error;
  ...
  _validateAll(context) {
    ...
    if (typeof this.props.validateAll === 'function') {
      const result = this.props.validateAll(context);
  ...
  _compileValidationRules(input, ruleProp) {
    const rules = ruleProp.split(',').map((rule) => {
  ...
  _handleSubmit(e) {
    ...
    const { isValid, errors } = this._validateAll(values);
